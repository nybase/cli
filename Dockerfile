...
RUN set -eux; addgroup -g 8080 app ; adduser -u 8080 -S -G app -s /bin/bash app ;\
    ...
    apk add --no-cache bash busybox-extras ca-certificates curl wget iproute2 iputils tzdata \
       tmux vi ttf-dejavu tcpdump 
      
RUN wget -P /tmp -c dl.k8s.io/${k8s}/bin/linux/${ARCH}/kubectl https://get.helm.sh/helm-${helm}-linux-${ARCH}.tar.gz \
